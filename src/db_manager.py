...
class DatabaseManager:
    ...
    def _load_sql_queries(self) -> Dict[str, str]:
        ...
        try:
            with open('queries.sql', 'r') as f:
                content = f.read()
                raw_queries = content.split('-- name:')[1:]
                for raw_query in raw_queries:
                    parts = raw_query.split('\n', 1)
                    queries[parts[0].strip()] = parts[1].strip()
        except FileNotFoundError: log.critical("queries.sql file not found."); raise
        return queries
    ...
    def _connect_with_retry(self):
        ...
        for i in range(retries):
            try:
                return ThreadedConnectionPool(1, 20, cursor_factory=DictCursor, host=self.host, port=self.port, user=self.user, password=self.password, dbname=self.dbname)
            except (psycopg2.OperationalError, psycopg2.errors.Error) as e:
                log.warning(f"Database connection failed: {e}. Retrying.")
                if i < retries - 1: time.sleep(delay); delay *= 2
                else: log.critical("Failed to connect to the database."); raise
    ...
    def get_patient_ids_by_identifiers(self, identifiers: List[Tuple[str, str, str]]) -> Dict[Tuple, int]:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("SELECT id, first_name, last_name, date_of_birth FROM patients WHERE (first_name, last_name, date_of_birth) IN %s;", (tuple(identifiers),))
                return {(row['first_name'], row['last_name'], str(row['date_of_birth'])): row['id'] for row in cur.fetchall()}
        finally: self.put_conn(conn)
            ...
            with conn.cursor() as cur:
                data_to_insert = [
                    (
                        ...
                        event.get('diagnoses'), # Use 'diagnoses' from CSV for 'assessment' column
                        None,  # 'subjective' not in CSV
                        None,  # 'objective' not in CSV
                        None,  # 'plan' not in CSV
                        event.get('clinical_notes'), # Use 'clinical_notes' from CSV for 'content' column
                        event['embedding']
                    )
    ...
    def insert_new_soap_note(self, patient_id: int, attending_physician: str, subjective: str, objective: str, assessment: str, plan: str, embedding: List[float]):
        conn = self.get_conn()
        try:
            with conn.cursor() as cur:
                rag_content = (f"Subjective: {subjective}\nObjective: {objective}\nAssessment: {assessment}\nPlan: {plan}")
                event_date = datetime.now().date()
                cur.execute(sql.SQL(self.queries['insert_new_soap_note']), (patient_id, event_date, attending_physician, subjective, objective, assessment, plan, rag_content, embedding))
            conn.commit()
            log.info(f"Successfully inserted new SOAP note for patient_id: {patient_id}")
        except Exception as e: log.error(f"Error inserting new SOAP note: {e}"); conn.rollback(); raise
        finally: self.put_conn(conn)
    ...
    def find_patient_by_details(self, first_name: str, last_name: str, dob: str) -> int | None:
        ...
        try:
            with conn.cursor() as cur:
                ...
                if len(results) == 1: return results[0]['id']
                else: log.warning(f"Ambiguous or no patient found for {first_name} {last_name}"); return None
        finally: self.put_conn(conn)
    ...
    def log_query(self, patient_id: int, question: str, context: str, answer: str):
        ...
        except Exception as e: log.error(f"Error logging query: {e}"); conn.rollback()
        finally: self.put_conn(conn)
    ...
    def log_action(self, doctor_id: int, action: str, patient_id: int | None = None):
        ...
        except Exception as e: log.error(f"Error creating audit log: {e}"); conn.rollback()
        finally: self.put_conn(conn)