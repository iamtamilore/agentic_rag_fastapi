...
class DatabaseManager:
    ...
    def _load_sql_queries(self):
        ...
        try:
            ...
            log.critical(
                "Failed to decode JSON from queries.sql. Attempting to parse as text."
            )
            with open(queries_path, "r") as f:
                raw = f.read()
            return self._parse_sql_queries(raw)
    ...
    def insert_patients(self, patients_data: List[Tuple]):
        ...
        try:
            with conn.cursor() as cur:
                execute_values(
                    cur,
                    sql.SQL(self.queries["insert_patient"]),
                    patients_data,
                    page_size=100,
                )
            conn.commit()
    ...
    def get_patient_ids_by_identifiers(
    ...
    def insert_medical_events(self, medical_events: List[Dict[str, Any]]):
        ...
        try:
            ...
            conn.commit()
            log.info(
                f"Successfully processed {len(medical_events)} medical event records."
            )
        finally:
            self.put_conn(conn)
    ...
    def insert_new_soap_note(
        ...
        try:
            ...
            conn.commit()
            log.info(
                f"Successfully inserted new SOAP note for patient_id: {patient_id}"
            )
    ...
    def find_similar_chunks(
        self, patient_id: int, query_embedding: List[float], k: int = 5
    ) -> List[Dict[str, Any]]:
        conn = self.get_conn()
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL(self.queries["find_similar_chunks"]),
                    (patient_id, str(query_embedding), k),
                )
                return cur.fetchall()
        finally:
            self.put_conn(conn)

    def find_patient_by_details(
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL(self.queries["find_patient_by_details"]),
                ...
                if len(results) == 1:
                    ...
                    log.warning(
                        f"Ambiguous or no patient found for {first_name} {last_name}"
                    )
                    return None
        finally:
            self.put_conn(conn)
    ...
    def get_doctor_by_username(self, username: str) -> dict | None:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL(self.queries["get_doctor_by_username"]), (username,)
                )
                return cur.fetchone()
        finally:
            self.put_conn(conn)
    ...
    def log_query(self, patient_id: int, question: str, context: str, answer: str):
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL(self.queries["insert_query_log"]),
                    (patient_id, question, context, answer),
                )
            conn.commit()
    ...
    def log_action(self, doctor_id: int, action: str, patient_id: int | None = None):
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    sql.SQL(self.queries["insert_audit_log"]),
