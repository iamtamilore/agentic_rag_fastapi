...
db_manager = DatabaseManager(host=os.getenv("DB_HOST", "db"), port=os.getenv("DB_PORT", "5432"), user=os.getenv("DB_USER", "rag_user"), password=os.getenv("DB_PASSWORD", "password123"), dbname=os.getenv("DB_NAME", "rag_db"))
...
def redact_pii(text: str, patient_name: str) -> str:
    if patient_name:
        return re.sub(r'\b' + re.escape(patient_name) + r'\b', '[PATIENT_NAME]', text, flags=re.IGNORECASE)
    return text

# --- API Data Models ---
class Token(BaseModel): access_token: str; token_type: str
class Doctor(BaseModel): id: int; username: str; full_name: str; role: str
class QueryRequest(BaseModel): patient_id: int; question: str
class QueryResponse(BaseModel): answer: str; retrieved_context: list[str]
class PatientLookupRequest(BaseModel): first_name: str; last_name: str; date_of_birth: str
class PatientLookupResponse(BaseModel): patient_id: int; message: str
class SOAPNoteRequest(BaseModel): subjective: str; objective: str; assessment: str; plan: str
class SOAPNoteResponse(BaseModel): success: bool; message: str
...
async def get_current_doctor(token: Annotated[str, Depends(oauth2_scheme)]) -> Doctor:
    ...
    if doctor_data is None: raise credentials_exception
    return Doctor(**doctor_data)

# --- API Endpoints ---
@app.get("/")
def read_root(): return {"message": "Welcome to the Clinical RAG Co-pilot API"}
...
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    access_token = create_access_token(data={"sub": doctor['username'], "role": doctor['role']})
...
async def find_patient(request: PatientLookupRequest, current_doctor: Annotated[Doctor, Depends(get_current_doctor)]):
    patient_id = db_manager.find_patient_by_details(request.first_name, request.last_name, request.date_of_birth)
    if patient_id is None: raise HTTPException(status_code=404, detail="Patient not found or details are ambiguous.")
    db_manager.log_action(doctor_id=current_doctor.id, action=f"SEARCHED_FOR_PATIENT", patient_id=patient_id)
    return PatientLookupResponse(patient_id=patient_id, message=f"Patient found by Dr. {current_doctor.full_name}.")
...
async def ask_question(request: QueryRequest, current_doctor: Annotated[Doctor, Depends(get_current_doctor)]):
    ...
    patient_name = f"{patient_info['first_name']} {patient_info['last_name']}" if patient_info else ""
...
async def create_soap_note(patient_id: int, request: SOAPNoteRequest, current_doctor: Annotated[Doctor, Depends(get_current_doctor)]):
    try:
        ...
        db_manager.log_action(doctor_id=current_doctor.id, action="CREATED_SOAP_NOTE", patient_id=patient_id)
        return SOAPNoteResponse(success=True, message="New SOAP note successfully saved.")
